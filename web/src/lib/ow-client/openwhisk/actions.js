...
export class Actions extends Resources {
	constructor(client) {
		super(client);
	...
	actionBody(options) {
		...
		if (isCodeAction && typeof options.action === 'object' && !(options.action instanceof Buffer)) {
		...
		const body = isCodeAction
			...
			: isSequenceAction
			? this.actionBodyWithSequence(options)
			: {};

		if (typeof options.params === 'object') {
			body.parameters = Object.keys(options.params).map((key) => ({
				key,
