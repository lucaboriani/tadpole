...
export class Namespaces extends BaseOperation {
	list(options) {
	...
	get(options) {
		...
		return Promise.all([actions, packages, triggers, rules]).then(
			([actions, packages, triggers, rules]) => ({ actions, packages, triggers, rules })
		);
	}
}
