...
import { OpenWhiskError } from './openwhisk_error';
import needle from '../needle/needle';
import url from 'node:url';
import * as http from 'http';
import { retry } from '../retry/async-retry';
...
const rp = (opts) => {
	...
	return needle(
		opts.method.toLowerCase(), // needle takes e.g. 'put' not 'PUT'
		url,
...
export class Client {
	...
	parseOptions(options) {
		...
		if (!apiKey && !options.auth_handler) {
			throw new Error(
				`${messages.INVALID_OPTIONS_ERROR} Missing api_key parameter or token plugin.`
			);
		} else if (!api) {
			throw new Error(
				`${messages.INVALID_OPTIONS_ERROR} Missing either api or apihost parameters.`
			);
		}
		...
		if (retry && typeof options.retry !== 'object') {
			throw new Error(
				`${messages.INVALID_OPTIONS_ERROR} 'retry' option must be an object, e.g. '{ retries: 2 }'.`
			);
		}
		if (retry) {
		...
		return {
			...
			apigwSpaceGuid: apigwSpaceGuid,
			authHandler: options.auth_handler,
			noUserAgent: options.noUserAgent,
			cert: options.cert,
			key: options.key,
			proxy,
			agent,
	...
	apiUrl() {
		return url.parse(this.options.api.endsWith('/') ? this.options.api : this.options.api + '/');
	}
	...
	handleErrors(reason) {
		...
		if (reason.hasOwnProperty('statusCode')) {
			...
			message = `${reason.options.method} ${reason.options.url} Returned HTTP ${
				reason.statusCode
			} (${http.STATUS_CODES[reason.statusCode]}) --> "${responseError}"`;
		}
