...
Needle.prototype.start = function () {
	...
	var json =
		options.json ||
		(options.json !== false && config.headers['content-type'] == 'application/json');
...
export default function needle() {
	...
	if (verb.match(/get|head/) && args.length == 1) args.splice(1, 0, null); // assume no data if head/get with one argument (options)
