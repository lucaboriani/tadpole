...
declare namespace openwhisk {
	...
	interface Actions {
		...
		invoke(
			options: (
				| string
				| { name: string; namespace?: string; blocking?: boolean; params?: Dict; result?: boolean }
			)[]
		): Promise<{ activationId: string }[]>;
		invoke<In extends Dict, Out extends Dict>(options: {
			name: string;
			namespace?: string;
			blocking: boolean;
			params?: In;
	...
	interface Triggers {
		...
		invoke(
			options: (string | { name: string; namespace?: string; params?: Dict })[]
		): Promise<{ activationId: string }[]>;
		invoke<T extends Dict>(options: {
			name: string;
			namespace?: string;
			params?: T;
		}): Promise<{ activationId: string }>;
	...
	interface Feeds {
		create(options: {
			...
			namespace?: string;
			params?: Dict;
		}): Promise<Activation<FeedActionDesc>>;
		delete(options: {
			name: string;
			trigger: string;
			namespace?: string;
		...
		feed(
			event: Event,
			options: { name: string; trigger: string; namespace?: string }
		): Promise<Activation<FeedActionDesc>>;
	}
	...
	type Status = '' | 'active' | 'inactive' | 'activating' | 'deactivating';

	type Event = '' | 'CREATE' | 'DELETE' | 'PAUSE' | 'UNPAUSE';

	type Operation = '' | 'GET' | 'POST' | 'PUT' | 'PATCH' | 'DELETE';
